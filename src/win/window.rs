...
impl WindowHandle {
    ...
    pub fn resize(&self, size: Size) {
        if let Some(hwnd) = self.hwnd {
            ...
            unsafe {
                SetWindowPos(hwnd, std::ptr::null_mut(), 0, 0, physical_width as i32, physical_height as i32, SWP_NOMOVE | SWP_NOZORDER | SWP_NOOWNERZORDER);
            }
        }
    }
...
unsafe fn register_wnd_class() -> ATOM {
    ...
    let wnd_class = WNDCLASSW {
        ...
        hbrBackground: CreateSolidBrush(RGB(32, 32, 32)), // Set background color to prevent flickering while opening window; TODO: Make color configurable?
...
impl Window {
    ...
    pub fn set_mouse_cursor(&mut self, cursor: MouseCursor) {
        unsafe {
            ...
            // Setting the window class' cursor instead of setting the global cursor using SetCursor() makes sure
            // that this cursor is only applied within the client bounds of our window - see docs of SetCursor().
            SetClassLongPtrW(self.hwnd, GCLP_HCURSOR, std::mem::transmute(cursor));
        }
    }
